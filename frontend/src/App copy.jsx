...
export default function FlightRouteMap() {
  ...
  const [preferredOption, setPreferredOption] = useState("cost"); // 'cost' or 'time' (distance)
  ...
  // Fetch airports and routes on component mount
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const g = zoomGroup.append("g"); // g is now a child of zoomGroup

    // Add route connections (lines)
    const routeLines = g
    ...
    // Highlight the calculated path if available
    if (path.length > 1) {
    ...
    const zoom = d3
      ...
      .scaleExtent([0.5, 5])
      .on("zoom", ({ transform }) => {
        zoomGroup.attr("transform", transform); // Apply transform to zoomGroup
      });
  ...
  const handleAirportClick = (airport) => {
    if (!sourceAirport) {
      ...
      // Find path after selecting destination
  ...
  // Toggle preference between cost and time (distance)
  const togglePreference = () => {
    const newPreference = preferredOption === "cost" ? "distance" : "cost";
    setPreferredOption(newPreference);

    // Recalculate path if both airports are selected
    if (sourceAirport && destAirport) {
      findShortestPath(sourceAirport.id, destAirport.id);
    }
  };

  // Call backend API to find the shortest path
  const findShortestPath = async (sourceId, destId) => {
    try {
      setLoading(true);
      setError(null);
